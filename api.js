...
async function initializeDatabase() {
  try {
    ...
    try {
      ...
      `);
      console.log("✅ MySQL Chat Tables Created/Checked");
    } finally {
      connection.release();
    }
  } catch (error) {
...
app.post("/api/chat", async (req, res) => {
  ...
  const endpoint =
    process.env.AIML_API_ENDPOINT ||
    "https://api.aimlapi.com/v1/chat/completions"; // Use env var for endpoint
  ...
  try {
    const aiApiResponse = await axios.post(
      endpoint,
      {
        model: process.env.AIML_API_MODEL || "gpt-4o-mini", // Use env var for model
        messages: [
          { role: "system", content: systemPrompt },
          { role: "user", content: userMessage },
        ],
        temperature: parseFloat(process.env.AIML_API_TEMPERATURE || "0.7"), // Use env var for temperature
        max_tokens: parseInt(process.env.AIML_API_MAX_TOKENS || "256"), // Use env var for max tokens
      },
      {
        headers: {
    ...
    const aiResponse =
      aiApiResponse.data.choices?.[0]?.message?.content ||
      "I'm still learning.";
    ...
    try {
      ...
      if (!currentSessionId) {
        ...
        if (sessionExists.length === 0) {
          return res
            .status(400)
            .json({ error: "Invalid sessionId for this user." });
        }
      }

      await connection.execute(
        "INSERT INTO chat_messages (session_id, sender, text) VALUES (?, ?, ?)",
        [currentSessionId, "You", userMessage]
      );
      await connection.execute(
        "INSERT INTO chat_messages (session_id, sender, text) VALUES (?, ?, ?)",
        [currentSessionId, "TAL AI", aiResponse]
      );
  ...
  try {
    ...
    try {
      ...
      const sessionList = sessions.map((session) => ({
        ...
        summary:
          session.first_user_message?.substring(0, 50) + "..." || "New Chat",
      }));
  ...
  try {
    ...
    try {
      ...
      if (sessionExists.length === 0) {
        return res
          .status(404)
          .json({ error: "Session not found for this user." });
      }
